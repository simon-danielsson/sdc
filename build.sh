...
compile() {
    ...
    if [[ "$BUILD_TYPE" == "release" ]]; then
        ...
        $compiler "${compilation_flags_constant[@]}"             "${compilation_flags_release[@]}"             "${SRC_FILES[@]}"             -o "$DIR_BIN"/"$BIN_NAME"

    elif [[ "$BUILD_TYPE" == "test" ]]; then
        log_details "flags: ${compilation_flags_constant[*]} ${compilation_flags_debug[*]}"

        $compiler "${compilation_flags_constant[@]}"            "${compilation_flags_debug[@]}"            "${SRC_FILES[@]}"             -o "$DIR_BIN"/"$BIN_NAME"
